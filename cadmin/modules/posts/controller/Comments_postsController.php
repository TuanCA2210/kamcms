...
class Comments_postsController extends Controller{
	...
	public function index(){
		...
		if (isset($_GET['s']) && $_GET['s']!='') {
			...
			$this->view->data['data'] = $this->modelComments_posts->findSearch($search);
			$this->view->data['s'] = $search;
			$this->view->data['curpage'] = 1;
			$this->view->data['count_page'] = 1;
			$this->view->data['pagination'] ='';
		}else{
			$link = base_url().'posts/comments_posts/index';
			$all_pages = $this->modelComments_posts->getComments();
			...
			$this->view->data['data'] = $this->modelComments_posts->getPagiComments($start,$limit);
