	...
	$("#list-feature-home").sortable({
	  	...
	  	update : function (e,ui) {
	    	var order = $('#list-feature-home').sortable('serialize');
	     	...
	     	$.ajax({
	     		url: baseUrl+'product/manager/sortable?'+order,
	     		type: 'GET',
	     		dataType: 'json'
	     	})
	     	.done(function(data) {
	...
	if (str!="") {
		...
		.done(function(data) {
			if (data.status==true) {
				...
				$.ajax({
					url: baseUrl+'product/manager/getOption',
					type: 'POST',
					dataType: 'html',
					data: {param: 1},
				})
...
$('body').on('click', '#del_list_manager', function(event) {
	...
	$('body').on('click', '#modelDeleteAll #agree_del_all_manager', function(event) {
		...
		if (allVals.length >= 1) {
			$.ajax({
				url: baseUrl+'product/manager/dellAll',
				type: 'POST',
				dataType: 'json',
				data: {all: allVals},
			})
	...
	$('body').on('click', '#lock_manager', function(event) {
		...
		if (allVals.length >= 1) {
			var status = 'public';
			$.ajax({
				url: baseUrl+'product/manager/status',
				type: 'POST',
				dataType: 'json',
	...
	$('body').on('change', '#cate_product_left', function(event) {
		...
		$.ajax({
			url: baseUrl+'product/manager/getProductByCate',
			type: 'POST',
			dataType: 'json',
			data: {id: id_cate},
		})
		.done(function(data) {
			if (data.status==true) {
				...
				var fruits = [];
				$('#list-all-product-manager .row .col-md-3').each(function(index, el) {
	...
	$('body').on('click', '#load-more-all-data-product', function(event) {
		...
		$.ajax({
			url: baseUrl+'product/manager/ajaxLoadMore',
			type: 'POST',
			dataType: 'json',
			data: {data},
		})
	...
	$('body').on('click', '#add_product_home', function(event) {
		...
		var fruits = [];
		$('#list-all-product-manager .row .col-md-3').each(function(index, el) {
			var id_d = $(this).find('.active').parents('.col-md-3').attr('data-id');
			if (id_d!=undefined) {
				fruits.push(id_d);
			}
		});
		...
		if (status==true) {
			$.ajax({
				url: baseUrl+'product/manager/ajaxAddProductHome',
				type: 'POST',
				dataType: 'json',
